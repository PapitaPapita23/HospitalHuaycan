...
interface ArchivoInicioResumenProps {
  resumen: DashboardResumenDTO;
}

export default function ArchivoInicioResumen({ resumen }: ArchivoInicioResumenProps) {
  return (
    <div className="relative overflow-hidden rounded-2xl bg-[#0A1733] p-6">
      ...
      <div className="relative z-10 mb-4 flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10">
          <IoFolderOpenOutline className="h-5 w-5 text-white" />
        </div>
        <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-white/50">
          Documentos digitalizados · hoy
        </p>
      </div>
      <p className="relative z-10 mb-4 text-[3.5rem] font-black leading-none text-white">
        {resumen.totalDocumentosEscaneadosHoy}
      </p>
      <p className="relative z-10 text-xs font-semibold text-white/50">
        {resumen.totalDocumentosEscaneados} en total en el archivo
      </p>
    </div>
  );
}
